"""
IndiGo Pricing Intelligence Dashboard v6
Team 5 — ISB Action Learning Project 2026

CONFIG LIVES IN STREAMLIT SECRETS — NOT IN THIS FILE.
Pasting this file over GitHub will never wipe your API key or sheet name again.
...
# ── CONFIG FROM SECRETS (never hardcoded) ────────────────────
...
/* Top padding matters — zero pushes content under Streamlit's fixed toolbar */
.block-container{padding:2.6rem 1.4rem 2rem !important;max-width:100% !important;}
...
.pid-title{font-size:1.2rem;font-weight:700;color:#fff;letter-spacing:-0.01em;line-height:1.2;}
...
.pid-ctx-val{font-size:0.9rem;font-weight:700;color:#fff;font-family:'DM Mono',monospace;line-height:1.1;}
.pid-ctx-lbl{font-size:0.54rem;color:rgba(255,255,255,0.6);text-transform:uppercase;letter-spacing:0.07em;}
...
.live-pill{background:rgba(255,255,255,0.14);border:1px solid rgba(255,255,255,0.3);
  color:#8affc0;border-radius:20px;padding:0.22rem 0.7rem;font-size:0.6rem;font-weight:700;
  display:flex;align-items:center;gap:0.35rem;letter-spacing:0.06em;}
.live-dot{width:6px;height:6px;background:#2ecc71;border-radius:50%;animation:blink 1.8s infinite;}
@keyframes blink{0%,100%{opacity:1}50%{opacity:0.25}}

/* ── Section labels ── */
.sec-hd{font-size:0.63rem;font-weight:700;letter-spacing:0.14em;text-transform:uppercase;
  color:#1B2D6B;margin:0.2rem 0 0.6rem 0;display:flex;align-items:center;gap:0.5rem;}
.sec-hd::after{content:'';flex:1;height:1px;background:#dde3f0;}

/* ── KPI strip: full width, 5 across ── */
.kpi-strip{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:0.8rem;margin-bottom:0.4rem;}
.kpi-card{background:#fff;border:1px solid #dde3f0;border-radius:11px;padding:0.85rem 1rem;
  box-shadow:0 1px 7px rgba(27,45,107,0.06);min-width:0;}
.kpi-card.accent{border-left:3px solid #E91E8C;}
.kpi-val{font-size:1.45rem;font-weight:700;color:#1B2D6B;font-family:'DM Mono',monospace;
  line-height:1.1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.kpi-lbl{font-size:0.58rem;color:#6a80ad;text-transform:uppercase;letter-spacing:0.09em;margin-top:0.3rem;}
.kpi-sub{font-size:0.66rem;color:#8095bd;margin-top:0.12rem;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
.k-green{color:#16A34A !important;}
.k-amber{color:#D97706 !important;}
.k-red{color:#DC2626 !important;}
.k-mag{color:#E91E8C !important;}
.k-navy{color:#1B2D6B !important;}

/* ── Fare table ── */
.fare-tbl{width:100%;border-collapse:separate;border-spacing:0;font-size:0.74rem;
  border:1px solid #dde3f0;border-radius:10px;overflow:hidden;table-layout:fixed;background:#fff;}
...
.date-sep td{background:#eaf0fb !important;color:#1B2D6B !important;
  ...
  border-top:2px solid #c9d6f0 !important;letter-spacing:0.02em;}
...
.f-exp{color:#DC2626 !important;}
.f-sim{color:#D97706 !important;}
.lf-g{color:#16A34A;font-weight:600;}
.lf-a{color:#D97706;font-weight:600;}
.lf-r{color:#DC2626;font-weight:600;}

/* ── AI panel ── */
.ai-wrap{background:#fff;border:1px solid #dde3f0;border-radius:12px;
  padding:1rem 1.2rem;box-shadow:0 1px 7px rgba(27,45,107,0.06);margin-bottom:0.6rem;}
...
.ai-badge-ok{display:inline-block;background:#dcfce7;border:1px solid #16A34A;color:#15803d;
  font-size:0.68rem;font-weight:700;padding:0.18rem 0.65rem;border-radius:20px;}
.ai-badge-ov{display:inline-block;background:#fef3c7;border:1px solid #D97706;color:#b45309;
  font-size:0.68rem;font-weight:700;padding:0.18rem 0.65rem;border-radius:20px;}
.ai-price{font-size:2.1rem;font-weight:700;color:#1B2D6B;font-family:'DM Mono',monospace;
...
.bd-row{display:flex;justify-content:space-between;border-bottom:1px dashed #e6ebf7;padding:0.03rem 0;}
...
.flt-pill{background:#f2f5fc;border:1px solid #c9d6f0;border-radius:9px;
  padding:0.5rem 0.8rem;font-size:0.73rem;color:#2a4060;line-height:1.7;margin-bottom:0.6rem;}
.flt-pill-t{font-size:0.9rem;font-weight:700;color:#1B2D6B;font-family:'DM Mono',monospace;}
...
.stRadio>div>label{color:#2a4060 !important;font-size:0.75rem !important;text-transform:none !important;
  letter-spacing:0 !important;font-weight:500 !important;background:#fafbff;
  border:1px solid #c9d6f0;border-radius:6px;padding:0.2rem 0.65rem;}
...
div[data-testid="metric-container"]{background:#fff !important;border:1px solid #dde3f0 !important;
  border-radius:10px;padding:0.65rem 0.9rem;box-shadow:0 1px 6px rgba(27,45,107,0.05);}
div[data-testid="metric-container"] label{color:#6a80ad !important;font-size:0.6rem !important;
...
.stDateInput>div>div>input{background:#fafbff !important;border:1px solid #c9d6f0 !important;
  color:#1a2740 !important;border-radius:8px !important;}
...
def load_data():
    ...
    if "Seats Sold" in indigo_df.columns:
        indigo_df["Seats Sold"]=pd.to_numeric(indigo_df["Seats Sold"],errors="coerce")
...
def load_feedback():
    try:
        data=get_sheet_client().worksheet(FEEDBACK_TAB).get_all_records()
...
def load_ai_log():
    try:
        data=get_sheet_client().worksheet(AI_LOG_TAB).get_all_records()
...
def _append(tab,hdrs,row):
    ...
    try:
        ...
        ws=sheet.add_worksheet(tab,rows=5000,cols=len(hdrs)+2)
        ws.append_row(hdrs)
...
# ── PRICING LOGIC (mirrors the Excel MVP) ────────────────────
def calc_fare(route,cabin,days_to_dep,load_factor,best_comp_fare,
    ...
    cab=  {"Economy":0.0,"Premium Economy":0.50,"Business":0.80}.get(cabin,0.0)
...
def call_gemini(route,flight_no,dep_time,cabin,dep_date,days_to_dep,load_factor,
    ...
    if not GEMINI_API_KEY:
        return None,None,None,("No API key found. Add GEMINI_API_KEY to Streamlit "
                               "Secrets (Manage app → Settings → Secrets).")

    comp_text="\n".join([f"  {a} ({fn} {ft}): ₹{fare:,}" for a,fn,ft,fare in comp_fares]) \
...
Rationale: [2-3 plain English sentences]"""
    ...
    try:
        ...
        return None,None,None,f"Could not reach Gemini: {e}"

    if resp.status_code!=200:
        detail=resp.text[:400] if resp.text else "(no response body)"
        return None,None,None,f"Gemini returned HTTP {resp.status_code}. {detail}"

    try:
        text=resp.json()["candidates"][0]["content"]["parts"][0]["text"].strip()
    except Exception:
        return None,None,None,f"Unexpected Gemini response: {resp.text[:300]}"

    decision,fare,rationale="Approve",arithmetic_fare,""
    for line in text.split("\n"):
        ls=line.strip()
        if ls.lower().startswith("decision:"):
            decision=ls.split(":",1)[1].strip()
        elif ls.lower().startswith("suggested fare:"):
            raw=ls.split(":",1)[1]
            digits="".join(c for c in raw if c.isdigit())
            if digits: fare=int(digits)
        elif ls.lower().startswith("rationale:"):
            rationale=ls.split(":",1)[1].strip()
    if not rationale:
        rationale=text[:300]
    return decision,fare,rationale,None
...
def main():
    try:
        ...
        st.info("Check that GOOGLE_SHEET_NAME and [gcp_service_account] are set in "
                "Streamlit Secrets, and that the sheet is shared with the service account.")
        return

    today=pd.Timestamp.today().normalize()
    dmin=today-timedelta(days=30)
    dmax=today+timedelta(days=90)

    # ── SIDEBAR ─────────────────────────────────────────────
    with st.sidebar:
        st.markdown('<div class="sb-brand"><span style="color:#E91E8C;font-weight:800">6E</span>'
        ...
        st.markdown('<p style="color:#1B2D6B;font-size:0.62rem;font-weight:700;'
                    'text-transform:uppercase;letter-spacing:0.09em;margin:0.6rem 0 0.15rem">'
                    'Departure Date</p>',unsafe_allow_html=True)
        d1=st.date_input("Dep",value=today.date(),min_value=dmin.date(),
                         max_value=dmax.date(),label_visibility="collapsed",key="d1")
        if trip_type=="Round Trip":
            st.markdown('<p style="color:#1B2D6B;font-size:0.62rem;font-weight:700;'
                        'text-transform:uppercase;letter-spacing:0.09em;margin:0.6rem 0 0.15rem">'
                        'Return Date</p>',unsafe_allow_html=True)
            d2=st.date_input("Ret",value=(today+timedelta(days=7)).date(),
        ...
        if not GEMINI_API_KEY:
            st.warning("Gemini key missing — add GEMINI_API_KEY to Secrets.")
    ...
    <div class="pid-hdr">
      <div>
        <div class="pid-title"><span style="color:#ffd9ee;font-weight:800">6E</span>
    ...
    # ── FLIGHT SELECTOR (drives KPIs + AI panel) ────────────
    fopts=(indigo_f[["Flight No.","Departure Time","Departure Date","Days to Departure"]]
    ...
    labels=[f"{r['Flight No.']}  ·  {r['Departure Time']}  ·  "
            ...
            f"({r['Days to Departure']}d to departure)"
            for _,r in fopts.iterrows()]
    sel_label=st.selectbox("Select flight to analyse",labels)
    fr=fopts.iloc[labels.index(sel_label)]
    f_no=str(fr["Flight No."]); f_time=str(fr["Departure Time"])
    f_date=fr["Departure Date"]; f_days=int(fr["Days to Departure"])

    frow=indigo_f[(indigo_f["Flight No."]==f_no)&(indigo_f["Departure Date"]==f_date)]
    ...
    # Today's AI log entry for this flight
    ...
    with a_left:
        st.markdown('<div style="font-size:0.6rem;font-weight:700;color:#1B2D6B;'
                    ...
                    'How the arithmetic fare is built</div>',unsafe_allow_html=True)
        bd="<div class='arith-box'>"
        for k,v,lbl in breakdown:
            if k=="Base Fare":
                bd+=f'<div class="bd-row"><span>Base fare</span><span>{lbl}</span></div>'
            elif k=="Final Fare":
                bd+=(f'<div class="bd-row"><span>Arithmetic fare</span>'
    ...
    with a_right:
        ...
        if st.button("🤖  Get AI recommendation"):
            ...
            with st.spinner("Asking the AI pricing analyst..."):
                dec,fare,rat,err=call_gemini(sel_route,f_no,f_time,sel_cabin,
                                             str(f_date)[:10],f_days,f_lf,arith,
                                             comp_list,strategy,hist,pax_type,trip_type)
            if err:
                st.error(err)
            else:
                try:
                    save_ai_log({"Log Date":datetime.now().strftime("%Y-%m-%d"),
                        "Route":sel_route,"Flight No.":f_no,"Departure Time":f_time,
                        "Departure Date":str(f_date)[:10],"Cabin Class":sel_cabin,
                        "Days to Departure":f_days,"Load Factor":round(f_lf*100,1),
                        "Arithmetic Fare":arith,"AI Decision":dec,
                        "AI Suggested Fare":fare,"Manager Decision":"Pending",
                        "Final Fare Used":""})
                except Exception as e:
                    st.warning(f"Recommendation received but could not be logged: {e}")
                st.session_state["ai"]={"dec":dec,"fare":fare,"rat":rat,"arith":arith,
                    "flt":f_no,"time":f_time,"date":str(f_date)[:10],"days":f_days,
                    "lf":f_lf,"strategy":strategy}

        if "ai" in st.session_state:
            r=st.session_state["ai"]
            ok=r["dec"].lower().startswith("approve")
            badge="ai-badge-ok" if ok else "ai-badge-ov"
            text="✔ Approves arithmetic fare" if ok else "⚡ Overrides arithmetic fare"
            st.markdown(f"""
            <div class="ai-result">
              <div style="display:flex;align-items:center;justify-content:space-between;">
                <span style="font-size:0.6rem;font-weight:700;color:#1B2D6B;
                      text-transform:uppercase;letter-spacing:0.1em;">AI recommendation</span>
            ...
            strat2=st.selectbox("Revise strategic direction (optional)",STRATEGIC_OPTIONS,
            ...
            def commit(kind,final_fare):
                save_feedback({"Timestamp":datetime.now().strftime("%Y-%m-%d %H:%M"),
                    "Route":sel_route,"Flight No.":r["flt"],"Departure Time":r["time"],
                ...
                save_ai_log({"Log Date":datetime.now().strftime("%Y-%m-%d"),
                    ...
                    "Days to Departure":r["days"],"Load Factor":round(r["lf"]*100,1),
                    "Arithmetic Fare":r["arith"],"AI Decision":r["dec"],
                    "AI Suggested Fare":r["fare"],"Manager Decision":kind,
                    "Final Fare Used":final_fare})

            m1,m2=st.columns(2)
            with m1:
                if st.button("✔  Accept AI fare"):
                    try:
                        commit("Accepted",r["fare"])
                        st.success(f"Accepted at {inr(r['fare'])}.")
                        del st.session_state["ai"]; st.cache_data.clear(); st.rerun()
                    except Exception as e:
    ...
    if not feedback_df.empty and "Manager Decision" in feedback_df.columns:
        acc=feedback_df[feedback_df["Manager Decision"].isin(["Accepted","Overridden"])]
    ...
    <col style="width:13%"><col style="width:11%"><col style="width:8%"><col style="width:9%">
    ...
    for _,row in indigo_f.sort_values(["Departure Date","Departure Time"]).iterrows():
        dd=row["Departure Date"]; ds=str(dd)[:10]
        fno=str(row.get("Flight No.","")); ftm=str(row.get("Departure Time",""))
        slot=str(row.get("Time Slot","")); dout=row.get("Days to Departure","")
        lf=float(row.get("Load Factor",0) or 0)
        sold=int(row.get("Seats Sold",0) or 0); tot=int(row.get("Total Seats",180) or 180)
        hol=str(row.get("Holiday / Festival","No"))
        c,dot=lf_cls(lf)

        if dd is not cur and (cur is None or dd!=cur):
            cur=dd
        ...
        ar,_=calc_fare(sel_route,sel_cabin,int(dout) if dout else 30,lf,bc,
        ...
        if not qp_c.empty:
            ...
            if not m.empty: qpf=int(m.iloc[0]["Fare (INR)"])
    ...
    st.markdown(f"""<div class="legend-row">
      ...
      <span style="color:#0891b2">■</span> AI rec suggested, not yet accepted &nbsp;
    ...
    # ── 4a · Load factor over time, full booking history ────
    with c_left:
        st.markdown('<div class="sec-hd">Booking build-up over time</div>',unsafe_allow_html=True)

        flight_list=sorted(indigo_df[indigo_df["Route"]==sel_route]
                           .apply(lambda r:f"{r['Flight No.']} {r['Departure Time']}",axis=1)
                           .dropna().unique().tolist()) if not indigo_df.empty else []
        default_ix=flight_list.index(f"{f_no} {f_time}") if f"{f_no} {f_time}" in flight_list else 0
        pick=st.selectbox("Flight",flight_list,index=default_ix,key="lfpick") \
             if flight_list else None
        ...
        if hist.empty or dcol not in hist.columns:
            st.info("No booking history for this flight on the selected departure date.")
        else:
            hist=hist.dropna(subset=[dcol]).sort_values(dcol)
            hist=hist.groupby(dcol,as_index=False).last()
            hist["Load Factor"]=pd.to_numeric(hist["Load Factor"],errors="coerce")
            hist["Seats Sold"]=pd.to_numeric(hist["Seats Sold"],errors="coerce")
            hist["LF%"]=(hist["Load Factor"]*100).round(1)
            hist["New bookings"]=hist["Seats Sold"].diff().fillna(hist["Seats Sold"]).clip(lower=0)

            fig=make_subplots(rows=2,cols=1,shared_xaxes=True,
                              vertical_spacing=0.08,row_heights=[0.42,0.58])
            fig.add_trace(go.Bar(x=hist[dcol],y=hist["New bookings"],
                name="New bookings that day",marker_color="rgba(233,30,140,0.55)",
                hovertemplate="%{y:.0f} seats<extra></extra>"),row=1,col=1)
            fig.add_trace(go.Scatter(x=hist[dcol],y=hist["LF%"],
                name="Cumulative load factor",mode="lines+markers",
                line=dict(color=NAVY,width=2.5),marker=dict(size=5,color=NAVY),
                fill="tozeroy",fillcolor="rgba(27,45,107,0.07)",
                hovertemplate="%{y:.1f}%<extra></extra>"),row=2,col=1)
            fig.add_hline(y=85,line_dash="dot",line_color=RED,line_width=1,row=2,col=1)
            ...
            first=hist[dcol].min()
            st.caption(f"{pick} · tracked from {first.strftime('%d %b %Y')} "
                       f"({len(hist)} days of booking data) toward "
                       f"{pd.Timestamp(sel_dates[0]).strftime('%d %b %Y')} departure")

    # ── 4b · Price trend, all four lines ────────────────────
    with c_right:
        ...
        if not comp_df.empty and "Scrape Date" in comp_df.columns:
            ...
            if not ct.empty:
                g=(ct.groupby(["Scrape Date","Airline"])["Fare (INR)"]
                     .mean().reset_index())
                g.columns=["Date","Series","Fare"]
                frames.append(g)

        # IndiGo arithmetic fare recomputed for each historical day
        ...
        # IndiGo AI recommended fares from the log
        if not ai_log_df.empty and "Log Date" in ai_log_df.columns:
            ...
            if not al.empty and "AI Suggested Fare" in al.columns:
                al["AI Suggested Fare"]=pd.to_numeric(al["AI Suggested Fare"],errors="coerce")
        ...
        if frames:
            ...
            fig2=px.line(allt,x="Date",y="Fare",color="Series",markers=True,
                color_discrete_map={"Air India":SKY,"Akasa Air":RED,
                    "IndiGo arithmetic":MAGENTA,"IndiGo AI recommended":NAVY})
            for tr in fig2.data:
            ...
            st.caption("Solid = competitor fares scraped daily · dashed = IndiGo's own "
                       "arithmetic and AI recommended fares")
        else:
    ...
    if feedback_df.empty or "Manager Decision" not in feedback_df.columns:
        st.info("No manager decisions recorded yet. Accept or override an AI "
        ...
        if acc.empty:
            ...
            st.markdown("<br>",unsafe_allow_html=True)
            rp=(acc.groupby("Route")["Flight Profit"].sum().reset_index()
                   .sort_values("Flight Profit"))
            fig3=go.Figure(go.Bar(x=rp["Flight Profit"],y=rp["Route"],orientation="h",
                marker_color=[GREEN if x>0 else RED for x in rp["Flight Profit"]],
                text=[inr(x) for x in rp["Flight Profit"]],textposition="outside",
            ...
            cols=[c for c in ["Route","Flight No.","Departure Date","Cabin Class",
                "Passenger Type","Trip Type","Load Factor","Arithmetic Fare",
                "AI Suggested Fare","Final Fare Used","Manager Decision",
                "Profit Per Seat","Flight Profit"] if c in acc.columns]
            st.dataframe(acc[cols].sort_values("Departure Date",ascending=False),
                         use_container_width=True,hide_index=True)

    st.markdown("""<div style="margin-top:1.6rem;padding:0.6rem 0;border-top:1px solid #dde3f0;
      text-align:center;font-size:0.6rem;color:#8095bd;letter-spacing:0.08em;">
